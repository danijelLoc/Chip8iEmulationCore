...
public class Chip8EmulationCore: Chip8EmulationCoreProtocol {
    ...
    private var targetFrameTime: Double {
        1.0 / Double(systemScreenAndTimersFrequency)
    }
    private var systemCpuInstructionsCountPerFrame: Int {
        systemCpuFrequency / systemScreenAndTimersFrequency
    }

    @Published internal private(set) var hasEmulationStarted = false
    ...
    /// Output screen buffer 64 width x 32 height. Pixel can be 0 or 1. True is turned On and False is turned Off.
    /// One example of how to subscribe to this data is to create CGImage from it using fromMonochromeBitmap extension method and then show it in Image element.
    @Published internal private(set) var outputScreen: [Bool] = Array(
    ...
    /// Info about play status
    @Published internal private(set) var playingInfo: PlayingInfo = PlayingInfo(
    ...
    public init(
        ...
        logger: EmulationLoggerProtocol? = EmulationConsoleLogger()
    ) {
        self.logger = logger
    ...
    private func emulationLoop(program: Chip8Program) async throws {
        ...
        while !Task.isCancelled {
            ...
            let timeEnd = Date()
            let instructionsInterval: Double = timeEnd.timeIntervalSince(
                timeStart
            ).magnitude

            // To ensure constant target frame time. We have to introduce sleep interval.
            let sleepPeriod =
                targetFrameTime > instructionsInterval
                ? targetFrameTime - instructionsInterval : 0
            try? await Task.sleep(
                nanoseconds: UInt64(sleepPeriod * 1_000_000_000))
