...
public protocol Chip8EmulationCoreProtocol {
    ...
    var outputScreen: (screen: [Bool], fps: Double) { get }
    /// Info about play status
    var playingInfo: PlayingInfo { get }
    
    /// A publisher that emits changes to the debugSystemStateInfo value.
    ///
    /// This updates on every cpu cycle so be careful when observing it.
    var debugSystemState: Chip8SystemState? { get }
    /// Debug Info about encountered error
    var debugErrorInfo: Error? { get }

    /// Creates Chip8 System, loads the program and starts the emulation background task. Programs for Chip8 are executed indefinitely (infinite loop). Emulation task is saved for later cancelation in stop method.
    func startEmulation(_ program: Chip8Program) async
    
    /// Pause and resume the emulation of the Chip8 program
    func togglePause() async
    /// Stops emulation of the Chip8 program
    func stop() async
    /// Loads emulation state if it belongs to the current loaded program
    func loadState(_ newState: EmulationState) async
    ...
    func onKeyDown(_ key: Chip8Key) async
    /// Chip8 Gameplay key released. See Chip8Key enum for more information.
    func onKeyUp(_ key: Chip8Key) async
}
